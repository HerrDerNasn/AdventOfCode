...
def run_simulation(positions, velocities, steps=float('inf')):
    ...
    step_number = 0
    while step_number < steps and (
            not step_number or positions != original_positions or velocities != original_velocities):
        for i in range(len(positions)):
            velocities[i] += sum(
                1 if positions[i] < position else -1 for position in positions if position != positions[i])
        for i in range(len(positions)):
...
def part1(positions):
    ...
    return sum(
        (abs(px[i]) + abs(py[i]) + abs(pz[i])) * (abs(vx[i]) + abs(vy[i]) + abs(vz[i])) for i in range(len(positions)))
...
moon_positions = list(map(parse_point, open('input.txt', 'r').read().split('\n')))
